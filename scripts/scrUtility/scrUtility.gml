...
///@arg {horizontal_alignment} halign
///@arg {vertical_alignment} valign
function draw_set_align(halign, valign) {
	draw_set_halign(halign)
	draw_set_valign(valign)
}

///@func layer_tilemap_get_id_fixed(layer)
...
///@func sprite_calculate_xscale(ind)
function sprite_calculate_xscale(ind, w) {
	return w / sprite_get_width(ind)
}

///@func sprite_calculate_yscale(ind)
function sprite_calculate_yscale(ind, h) {
	return h / sprite_get_height(ind)
}
...
function position_make_relative(pos) {
	return new vec2(x + pos.x * image_xscale, y + pos.y * image_yscale)
}

function random_chance(chance) {
	return chance * 100 >= random(100)
}

///@func instance_create_depth_ext(x, y, depth, obj, spd, dir, grav, gravdir, xscl, yscl)
function instance_create_depth_ext(xx, yy, d, obj, spd, dir, grav, gravdir, xscl, yscl) {
	var _inst = instance_create_depth(xx, yy, d, obj)
	_inst.speed = spd
	_inst.direction = dir
	_inst.gravity = grav
	_inst.gravity_direction = gravdir
	_inst.image_xscale = xscl
	_inst.image_yscale = yscl
	return _inst
}
