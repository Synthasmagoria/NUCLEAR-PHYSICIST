...
function player_kill(inst_obj) {
	if (!global.debug_nodeath) {
		savedata_set_active("death", savedata_get_active("death") + (!savedata_get_active("clear")))
		...
		with (inst_obj) {
			instance_create_depth(x, y, 0, oBloodEmitter)
			instance_destroy()
		}
	}
}
	...
	with oPlayer {
		if wobj != weapon_object {
			...
			if wobj != -1 && wobj != undefined {
				weapon = instance_create_depth(x + hand.x, y + hand.y, global.player_depth + 1, wobj)
